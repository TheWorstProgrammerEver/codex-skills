---
name: coding-style
description: Apply Ryan Hayward's coding and review preferences. Use for fixes, refactors, accessibility, tests, security, host-diagnostic evidence, examples/fixtures, paginated authorization evidence, structured-config mutation, artifact conformance, lockfile portability, resumable artifact caches, remote promotion, launcher checks, bundle modes, stale locks, destructive block-device workflows, readiness-gated secret hydration, secret-bearing CLI isolation, temporary credential namespaces, prompt-driven external effects, atomic replacement, transactional sensitive-file migration, durable-state trust boundaries, recovery validation, derived-artifact replay, retry-budget, stable-clock, process-identity, Codex error-projection semantics, URL-to-filesystem validation, subprocess lifecycle, systemd timer re-arming and TTY routing, transient live checkpoint gates, cancellation settlement and orchestration propagation, GitHub Actions job containers, Bash, reviews, templates, React, TypeScript, and validation planning.
---
...
- `references/packaged-runtime-verification.md`: packaging native or foreign-architecture runtimes with independent archive, extracted-tree, executable identity and mode, placement-manifest, target-rescan, isolated service-unit checks, and installed system-service namespace validation.
