...
const ensureAppPortAvailable = async () => {
  ...
  if (await httpOk(appUrl)) {
    throw new Error(`Cannot start Team Tasks: ${appUrl} already responds, so ownership of the listener is ambiguous. Stop that listener explicitly, then retry get-going.`)
  }
}
...
const ensureVite = async () => {
  ...
  startManagedProcess(managedProcesses, 'Team Tasks dev server', 'npm', ['run', 'dev', '--', '--host', '0.0.0.0'])
  await waitFor('Team Tasks dev server', () => httpOk(appUrl))
}
