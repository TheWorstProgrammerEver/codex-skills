...
Press `Ctrl+C` to stop dev processes started by the script. Supabase containers keep their local data in Docker volumes; use `npm run all-done` when you want everything wound down. Lifecycle cleanup signals only this project's freshly revalidated `get-going` owner and waits for its isolated process groups; it never kills by shared port or command match. Failed child cleanup is retained on that runtime generation, and a terminal owner record is not discarded as descendant-cleanup proof. If an unowned listener or failed, stale, or ambiguous runtime remains, cleanup exits nonzero with bounded recovery guidance.
