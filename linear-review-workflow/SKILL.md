...
- When apply mode includes post-merge source-branch cleanup, make cleanup idempotent against repository auto-delete races. Re-read the merged PR after GitHub reports the merge, resolve the exact `head.repo.full_name` and `head.ref` from that response, and never derive the deletion target from memory, a branch prefix, or the local checkout. Delete only when the exact head repository is within the current authorized repository boundary, the ref is not the base branch, and it is not a fork branch, stacked branch, release branch, or other documented preservation branch unless a human explicitly authorized that exact deletion.
- For source-branch cleanup, check only the exact `heads/<head.ref>` in the resolved head repository. Treat an already absent ref as complete only after an authoritative exact ref read returns HTTP 404. If the exact DELETE succeeds, re-read the exact ref and require HTTP 404 before claiming cleanup. If the DELETE returns HTTP 422 such as `Reference does not exist`, accept it as an auto-delete race only when the follow-up exact ref read returns HTTP 404. Any permission error, transport error, mismatched repository/ref, or still-present ref after deletion means cleanup is incomplete and must be reported without broadening the target.
